// Package image provides an storage.Reader interface, along with various byte reading
...
func (r *Reader) SetEndian(typ Endian) {
	r.endianness = typ
}
...
// DWordBytesToLong converts a slice of 4 little endian ordered bytes (DWORD) to uint32.
func (r Reader) DWordBytesToLong(b [4]byte) uint32 {
	if r.endianness == LittleEndian {
		return binary.LittleEndian.Uint32(b[:])
	}
	return binary.BigEndian.Uint32(b[:])
...
// SwordBytesToLong converts a slice of 3 little endian ordered bytes (24-bit SWORD) to uint32.
func (r Reader) SwordBytesToLong(b [3]byte) uint32 {
